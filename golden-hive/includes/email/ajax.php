...
// ═══ DIAGNOSE ══════════════════════════════════════════════════════════════
//
// Non invia nulla. Esegue lo stesso checklist di rp_em_execute_campaign ma
// solo in lettura, e ritorna un report strutturato all'UI. Serve per capire
// perche "Invia ora" fallisce senza dover leggere debug.log.

add_action( 'wp_ajax_rp_em_ajax_campaign_diagnose', function () {
    rp_em_ajax_guard();
    $id = sanitize_text_field( (string) ( $_POST['id'] ?? '' ) );
    $c  = $id !== '' ? rp_em_get_campaign( $id ) : null;
    ...
    $report = [
        'campaign_id'        => $id,
        'campaign_exists'    => (bool) $c,
        'campaign_status'    => (string) ( $c['status'] ?? '' ),
        'source_type'        => (string) ( $c['source_type'] ?? '' ),
        'module_ids'         => array_map( 'intval', (array) ( $c['module_ids'] ?? [] ) ),
        'csv_has_content'    => $c ? ( trim( (string) ( $c['csv_contacts'] ?? '' ) ) !== '' ) : false,
    ...
    if ( ! $c ) {
        $report['blockers'][] = 'Campagna non trovata (id: ' . ( $id ?: '(vuoto)' ) . ').';
        wp_send_json_success( $report );
    }

    // Lock transient
    $lock_key              = 'rp_em_camp_lock_' . md5( $id );
    $report['lock_active'] = (bool) get_transient( $lock_key );

    // Template
    $template                 = $report['template_id'] !== '' ? rp_em_get_template( $report['template_id'] ) : null;
    $report['template_exists'] = (bool) $template;

    // Blocking validator
    $blocking                 = rp_em_validate_campaign_blocking( $id );
    $report['blocking_errors'] = array_map( fn( $e ) => [ 'code' => $e['code'] ?? '', 'key' => $e['key'] ?? '', 'message' => $e['message'] ?? '' ], $blocking['errors'] ?? [] );

    // Render
    try {
        $html = rp_em_render_campaign( $id );
        $report['render_ok']    = $html !== '';
        $report['render_bytes'] = strlen( (string) $html );
        if ( $html === '' ) $report['render_error'] = 'rp_em_render_campaign returned empty string';
    } catch ( \Throwable $e ) {
        $report['render_ok']    = false;
        $report['render_error'] = 'Exception: ' . $e->getMessage();
    }

    // Hustle environment
    global $wpdb;
    $et = $wpdb->prefix . 'hustle_entries';
    $mt = $wpdb->prefix . 'hustle_modules';
    $report['hustle_installed'] = $wpdb->get_var( "SHOW TABLES LIKE '{$et}'" ) === $et;
    if ( $report['hustle_installed'] ) {
        if ( $wpdb->get_var( "SHOW TABLES LIKE '{$mt}'" ) === $mt ) {
            $modules = rp_em_get_hustle_modules();
            foreach ( $modules as $m ) {
                $report['hustle_modules'][] = [
        ...
        $count = $wpdb->get_var( "SELECT COUNT(DISTINCT e.entry_id) FROM {$et} e" );
        $report['hustle_subs_total'] = (int) ( $count ?? 0 );
    }

    // Campaign contacts (stesso path di execute_campaign)
    try {
        $contacts = rp_em_resolve_campaign_contacts( $c );
        $report['contacts_count'] = count( $contacts );
        foreach ( array_slice( $contacts, 0, 5 ) as $row ) {
            $report['contacts_sample'][] = (string) ( is_object( $row ) ? ( $row->email ?? '' ) : ( $row['email'] ?? '' ) );
        }
    } catch ( \Throwable $e ) {
        $report['blockers'][] = 'Contacts resolver exception: ' . $e->getMessage();
    }

    // Sommario blockers: cosa fermerebbe Invia ora
    if ( ! empty( $report['blocking_errors'] ) ) {
        foreach ( $report['blocking_errors'] as $e ) {
            $report['blockers'][] = 'Validator: ' . $e['code'] . ' — ' . $e['message'];
        }
    }
    if ( $report['template_exists'] && ! $report['render_ok'] ) {
        $report['blockers'][] = 'Render fallito: ' . ( $report['render_error'] ?: 'sconosciuto' );
    }
    if ( $report['lock_active'] ) {
        $report['blockers'][] = 'Lock transient attivo: rp_em_camp_lock_' . md5( $id ) . ' — wp option/transient delete per sbloccare.';
    }
    if ( $report['contacts_count'] === 0 ) {
        $detail = $report['source_type'] === 'hustle' && ! $report['hustle_installed']
            ? ' (tabella wp_hustle_entries non esiste — plugin Hustle non installato o non attivo)'
            : '';
        $report['blockers'][] = 'Nessun contatto risolto dalla sorgente "' . $report['source_type'] . '"' . $detail . '.';
    }
